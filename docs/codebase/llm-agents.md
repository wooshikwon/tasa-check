...
`/check` 명령에서 호출되며, 수집된 기사 목록을 Claude API로 분석하여 [단독]/[주요]/[스킵]을 분류하고, 요약과 판단 근거를 생성한다.
...
**응답 파싱 로직:** `message.content`에서 `tool_use` 블록을 순회하며 `submit_analysis` 이름의 블록을 찾는다. `results`와 `skipped` 각각에서 `dict` 타입만 필터링하고, 타입 불일치 발생 시 경고 로그를 남긴다. `skipped` 항목에 `category: "skip"`을 주입한 뒤 `results + skipped`로 병합 반환한다.
...
5. **제외 기준:**
   ...
   - 재탕/종합 보도
   - 연예/스포츠 가십 (부서 무관)
6. **요약 작성 기준:** 2~3줄, 구체적 팩트 필수, 배경과 의미, 사실 기반

**시나리오 B 전용 -- [오늘 기존 캐시] 섹션:**
시스템 프롬프트에 기존 캐시 항목을 다음 형식으로 포함:
```
- id:{item_id} | {title} | 요약: {summary} | #{tag1} #{tag2}
```
...
```
- "{title}" | {summary} | #{tag1} #{tag2} | {category} | {created_at[:10]}
```
이력이 없으면 `"이력 없음. 모든 항목을 category: "new", prev_reference: null로 설정하라."` 명시.

**[오늘 기존 캐시 항목] (시나리오 B 전용):**
```
- id:{item_id} | {title} | 요약: {summary} | #{tag1} #{tag2}
```
  ...
  "input_schema": {
    ...
    "properties": {
      "results": {
        ...
        "items": {
          "type": "object",
          "required": ["title", "url", "source_indices", "summary",
                       "reason", "tags", "category", "exclusive", "prev_reference"],
          "properties": {
            "action": {
              "type": "string", "enum": ["modified", "added"],
            ...
            "reason": { "type": "string", "description": "선택 사유 1문장" },
            "tags": {
              "type": "array", "items": { "type": "string" },
              "description": "주제 태그 배열"
            },
            "category": {
              "type": "string", "enum": ["follow_up", "new"],
...
**시나리오 B에서의 action 판정:**
- `"modified"`: 기존 캐시 항목에 새 팩트가 추가된 경우. 기존 요약에 새 정보를 병합하며, `item_id`로 대상 항목을 특정한다.
- `"added"`: 기존 캐시에 없는 완전히 새로운 기사.
- `action`과 `item_id`는 `required`에 포함되지 않으며, 시나리오 A에서는 출력되지 않는다.
...
**산업부:**
- coverage: 대기업/중소기업, IT/반도체/배터리/자동차, AI/로봇/바이오, 스타트업, 통신, 에너지, 유통/물류, 건설/인프라, ESG/탄소중립
- criteria: 기업 공식 발표, 산업 정책, 기술 동향, 시장 변동 (4개)
- report_keywords: 13개
...
| Stage 2 | Sonnet (`claude-sonnet-4-5-20250929`) | 필터 통과 기사의 제목 + 본문 | 고비용 모델, 필터링된 입력 |
...
기사 본문은 스크래핑 시 최대 3문단만 추출한다 (프로젝트 커밋 이력 참조: "스크래퍼 본문 추출 2문단 -> 3문단으로 상향"). LLM에 전달되는 입력 토큰을 제한하면서도 기사의 핵심 정보를 충분히 파악할 수 있는 분량이다.

### 5.3 Temperature 0.0

check_agent와 report_agent 모두 Temperature를 0.0으로 설정하여 결정론적 출력을 유도한다. 뉴스 분석은 창의성보다 일관성과 재현성이 중요하므로, 동일 입력에 대해 동일한 분류/요약 결과를 보장한다.
...
|---|---|---|---|---|
| check_agent | `analyze_articles` | `"check_agent"` | `"span"` | `{"department": department}` |
| report_agent | `filter_articles` | `"report_filter"` | `"span"` | `{"department": department, "input_count": len(articles)}` |
| report_agent | `analyze_report_articles` | `"report_agent"` | `"span"` | `{"department": department, "scenario": scenario}` |

### 6.2 추적되는 정보

- `check_agent`: 부서명
- `report_filter`: 부서명 + 필터 입력 기사 수
- `report_agent`: 부서명 + 시나리오 (A 또는 B)
