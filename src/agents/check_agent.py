...
자기 검증: reason을 먼저 작성하라. reason이 위 제외 유형에 해당하면 해당 기사는 skip 대상이다.
reason에 '당일 보도', '오늘 뉴스' 등 수집 시점에 관한 당연한 사실은 쓰지 않는다. 왜 이 기사가 주요한지만 기재한다.

[3단계: 이전 보고 대비]
이전에 보고한 기사와 육하원칙(누가, 언제, 어디서, 무엇을)의 핵심 사실이 동일하면 skip한다.
