...
def _build_report_tool(is_scenario_b: bool) -> dict:
    ...
    if is_scenario_b:
        item_props["action"] = {
            "type": "string",
            "enum": ["modified", "added"],
            "description": "modified=<existing_items>의 동일 사안에서 새 육하원칙 뉴스 발견, added=<existing_items>에 없는 새로운 사안",
        }
        item_props["item_id"] = {
            "type": "integer",
            "description": "modified일 때 대상 기존 항목 순번 (<existing_items>의 항목 번호), added일 때 0",
        }
...
기사 본문에 "N일 A가 OO했다" 등 특정 주체의 오늘({today})자 팩트가 명시된 경우만 뉴스다.
  ...
  1-3) 단, 외신 인용("N일 로이터/NYT가 보도했다")이면, 전날 날짜의 팩트까지 당일 뉴스로 인정한다.
</step_1>
...
<step_3>
briefing_history 중복 제외
briefing_history와 중복된 내용의 기사는 다시 보고하지 않는다:
